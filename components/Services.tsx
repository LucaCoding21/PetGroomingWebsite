      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {SERVICES_DATA.map((service) => (
            <div
              key={service.id}
              className="group rounded-3xl overflow-hidden bg-white border border-brand-bg hover:border-brand-primary transition-all duration-500 hover:shadow-2xl hover:shadow-brand-primary/10 flex flex-col"
            >
              {/* Image Container */}
                ...
                <div className="mt-auto">
                  ...
                  <ul className="space-y-3 mb-8">
                    {service.features.map((feature, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm font-medium text-brand-text/80">
                  ...
                  <a href="#contact" className="w-full block py-3 text-center border border-brand-text rounded-xl text-sm font-bold uppercase tracking-wide hover:bg-brand-text hover:text-white transition-all">
        ...
        <div className="mt-20 flex flex-col items-center text-center">
          ...
          <a href={`tel:${BUSINESS.contact.phone}`} className="inline-flex items-center gap-2 text-2xl font-serif italic text-brand-text hover:text-brand-secondary transition-colors group">
            {COPY.services.ctaText}
            <span className="group-hover:translate-x-2 transition-transform"><ArrowRight /></span>
          </a>
        </div>
      </div>
    </section>
