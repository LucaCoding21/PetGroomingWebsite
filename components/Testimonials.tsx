import React from 'react';
import { Star, Plus, Minus, Quote } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { TESTIMONIALS_DATA, FAQ_DATA, COPY } from '../brand.config';
import { FadeIn, StaggerContainer, StaggerItem, ScaleIn } from './ScrollAnimations';

const Testimonials: React.FC = () => {
  const [openFaq, setOpenFaq] = React.useState<number | null>(0);

  return (
    <section id="faq" className="py-24 bg-brand-primary/5">
      <div className="max-w-7xl mx-auto px-6">

        {/* Reviews Header */}
        <div className="text-center mb-16">
          <FadeIn>
            <span className="text-brand-primary font-bold tracking-widest uppercase text-sm mb-4 block">{COPY.testimonials.label}</span>
          </FadeIn>
          <FadeIn delay={0.1}>
            <h2 className="font-serif text-4xl md:text-6xl text-brand-text mb-6">{COPY.testimonials.heading}</h2>
          </FadeIn>
          <ScaleIn delay={0.2}>
            <div className="inline-flex items-center gap-2 bg-white px-6 py-2 rounded-full shadow-sm">
              <div className="flex text-yellow-500">
                {[1,2,3,4,5].map(i => (
                  <motion.span
                    key={i}
                    initial={{ opacity: 0, scale: 0 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ delay: 0.3 + i * 0.1, type: 'spring' }}
                  >
                    <Star fill="currentColor" size={16} />
                  </motion.span>
                ))}
              </div>
              <span className="text-brand-text font-bold text-sm tracking-wide">{COPY.testimonials.ratingText}</span>
            </div>
          </ScaleIn>
        </div>

        {/* Reviews Grid */}
        <StaggerContainer staggerDelay={0.15} className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
          {TESTIMONIALS_DATA.map((t) => (
            <StaggerItem key={t.id}>
              <motion.div
                className="bg-white p-8 rounded-tr-3xl rounded-bl-3xl rounded-tl-lg rounded-br-lg shadow-lg hover:shadow-xl transition-shadow duration-300 relative group h-full"
                whileHover={{ y: -5, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.1)' }}
                transition={{ type: 'spring', stiffness: 300 }}
              >
                <motion.div
                  className="absolute -top-6 left-8 w-12 h-12 bg-brand-text text-white rounded-full flex items-center justify-center shadow-lg"
                  whileHover={{ scale: 1.1, rotate: 10 }}
                  transition={{ type: 'spring', stiffness: 300 }}
                >
                  <Quote size={20} />
                </motion.div>

                <div className="mt-4 mb-6">
                  <p className="text-brand-text/80 italic leading-relaxed text-lg">"{t.review}"</p>
                </div>

                <div className="flex items-center gap-4 border-t border-gray-100 pt-6">
                  <motion.img
                    src={t.image}
                    alt={t.name}
                    className="w-14 h-14 rounded-full object-cover ring-4 ring-brand-bg/30"
                    whileHover={{ scale: 1.1 }}
                  />
                  <div>
                    <h4 className="font-bold text-brand-text text-base">{t.name}</h4>
                    <div className="flex items-center gap-2 text-xs text-brand-text/60">
                      <span className="font-medium text-brand-primary">{t.petName}</span>
                      <span>•</span>
                      <span>{t.petType}</span>
                    </div>
                  </div>
                </div>
              </motion.div>
            </StaggerItem>
          ))}
        </StaggerContainer>

        {/* FAQ Section */}
        <FadeIn>
          <motion.div
            className="bg-white rounded-[3rem] p-8 md:p-16 shadow-xl"
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              <div>
                <FadeIn>
                  <span className="text-brand-secondary font-bold tracking-widest uppercase text-sm mb-4 block">{COPY.faq.label}</span>
                </FadeIn>
                <FadeIn delay={0.1}>
                  <h2 className="font-serif text-4xl text-brand-text mb-6">{COPY.faq.heading}</h2>
                </FadeIn>
                <FadeIn delay={0.2}>
                  <p className="text-brand-text/70 mb-8 leading-relaxed">
                    {COPY.faq.description}
                  </p>
                </FadeIn>
                <FadeIn delay={0.3}>
                  <motion.a
                    href="#contact"
                    className="inline-block px-8 py-3 bg-brand-text text-white rounded-full font-bold hover:bg-brand-secondary transition-colors shadow-lg hover:shadow-xl"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    {COPY.faq.ctaButton}
                  </motion.a>
                </FadeIn>
              </div>

              <div className="space-y-4">
                {FAQ_DATA.map((faq, index) => (
                  <motion.div
                    key={index}
                    className="border-b border-gray-100 pb-4 last:border-0"
                    initial={{ opacity: 0, x: 20 }}
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: index * 0.1 }}
                  >
                    <motion.button
                      className="w-full flex justify-between items-center py-4 text-left group"
                      onClick={() => setOpenFaq(openFaq === index ? null : index)}
                      whileHover={{ x: 5 }}
                      transition={{ type: 'spring', stiffness: 300 }}
                    >
                      <span className={`font-serif text-xl ${openFaq === index ? 'text-brand-primary' : 'text-brand-text'} transition-colors group-hover:text-brand-primary`}>
                        {faq.question}
                      </span>
                      <motion.span
                        className={`transition-colors duration-300 w-8 h-8 flex items-center justify-center rounded-full ${openFaq === index ? 'bg-brand-primary text-white' : 'bg-brand-bg/20 text-brand-text'}`}
                        animate={{ rotate: openFaq === index ? 180 : 0 }}
                        transition={{ duration: 0.3 }}
                      >
                        {openFaq === index ? <Minus size={16} /> : <Plus size={16} />}
                      </motion.span>
                    </motion.button>
                    <AnimatePresence>
                      {openFaq === index && (
                        <motion.div
                          initial={{ height: 0, opacity: 0 }}
                          animate={{ height: 'auto', opacity: 1 }}
                          exit={{ height: 0, opacity: 0 }}
                          transition={{ duration: 0.3, ease: [0.25, 0.4, 0.25, 1] }}
                          className="overflow-hidden"
                        >
                          <p className="text-brand-text/70 pb-4 leading-relaxed pl-2 border-l-2 border-brand-primary/20 ml-1">
                            {faq.answer}
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </motion.div>
                ))}
              </div>
            </div>
          </motion.div>
        </FadeIn>

      </div>
    </section>
  );
};

export default Testimonials;
